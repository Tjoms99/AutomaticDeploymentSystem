...
void work_rs485_write(struct k_work *work)
{
    char *message;
    ...
    while (k_msgq_get(&msgq_write, &message, K_NO_WAIT) == 0)
    {
        uart_write(message);
    }
...
void work_rs485_write_delayed(struct k_work *work)
{
    char *message;
    ...
    while (k_msgq_get(&msgq_write_delayed, &message, K_NO_WAIT) == 0)
    {
        uart_write(message);
    }
